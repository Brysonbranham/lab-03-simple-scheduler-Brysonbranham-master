      ...
      for (int i = 0; i <= num_tasks  ; i++ )
		{
			...
			if (list_of_tasks_to_do[i].remaining_time == 0 && list_of_tasks_to_do[i].suspended == false)
			{
				// I want to callback to the current task so I can run it. How do I callback that task
				task = list_of_tasks_to_do[i].callback;
				list_of_tasks_to_do[i].remaining_time = list_of_tasks_to_do[i].period;
			}
			if(!list_of_tasks_to_do[i].suspended)
			{
				list_of_tasks_to_do[i].remaining_time = list_of_tasks_to_do[i].remaining_time - runTime;
			}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
  ...
  /************** STUDENT EDITABLE HERE STARTS HERE *****/
	...
	case Button_1_Pin:
		...
		Scheduler_Toggle_Suspend(0);
		break;
	case Button_2_Pin:
		Scheduler_Toggle_Suspend(1);
		break;
	case Button_3_Pin:
		list_of_tasks_to_do[0].period = list_of_tasks_to_do[0].period + 3000;
		break;
	default:
      __NOP();
	}
